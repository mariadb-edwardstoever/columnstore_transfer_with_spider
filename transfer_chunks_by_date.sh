...
FIRST_REPORT_DATE_AS_UNIX_TIMESTAMP=$(echo $GET_DB_VALUES| awk '{print $2}'|xargs)
LAST_REPORT_DATE_AS_UNIX_TIMESTAMP=$(echo $GET_DB_VALUES| awk '{print $3}'|xargs)
COUNT_ALL_DAYS=$(echo $GET_DB_VALUES| awk '{print $4}'|xargs)
HOW_MANY_CHUNKS=$((($TOTAL_ROWS_SOURCE/$APPROX_ROWS_PER_CHUNK)+1))
DAYS_PER_CHUNK=$(($COUNT_ALL_DAYS/$HOW_MANY_CHUNKS))
echo "Starting to insert rows from table with Spider Engine."

ii=$FIRST_REPORT_DATE_AS_UNIX_TIMESTAMP; 
STEP=0;
while [[ $ii -lt $(($LAST_REPORT_DATE_AS_UNIX_TIMESTAMP+1)) ]]; do
  STARTSEC=$(date +%s)
  
  if [[ $ii -ge $LAST_REPORT_DATE_AS_UNIX_TIMESTAMP ]]; then OFFSET=0; else OFFSET=1; fi

  SQL="insert into $TARGET_SCHEMA.$TARGET_TABLE select * from $SPIDER_SCHEMA.$SPIDER_TABLE where $DATE_COLUMN
       between
       from_unixtime($ii) and from_unixtime($ii) + interval ($DAYS_PER_CHUNK - $OFFSET) day; select format(ROW_COUNT(),0);"

  ROWS_INSERTED=$(CStargetConn -ABNe "$SQL") || ERR=true
  if [ $ERR ]; then echo "Something went wrong during insert."; exit; fi
  ii=$(($ii + ($DAYS_PER_CHUNK * 60 * 60 * 24)))
  STEP=$(($STEP +1))
  ENDSEC=$(date +%s)
