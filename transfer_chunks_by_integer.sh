...
SQL="select count(*), min($INTEGER_COLUMN), max($INTEGER_COLUMN),(max($INTEGER_COLUMN)-min($INTEGER_COLUMN)) from main.amz_narrow_test;"

GET_DB_VALUES=$(CSsourceConn -ABNe "$SQL")
TOTAL_ROWS_SOURCE=$(echo $GET_DB_VALUES| awk '{print $1}' |xargs)
FIRST_REPORT_INTEGER=$(echo $GET_DB_VALUES| awk '{print $2}'|xargs)
LAST_REPORT_INTEGER=$(echo $GET_DB_VALUES| awk '{print $3}'|xargs)
COUNT_ALL=$(echo $GET_DB_VALUES| awk '{print $4}'|xargs)
HOW_MANY_CHUNKS=$((($TOTAL_ROWS_SOURCE/$APPROX_ROWS_PER_CHUNK)+1))
PER_CHUNK=$(($COUNT_ALL/$HOW_MANY_CHUNKS))
echo "Starting to insert rows from table with Spider Engine."

ii=$FIRST_REPORT_INTEGER; 
STEP=0;
while [[ $ii -lt $(($LAST_REPORT_INTEGER+1)) ]]; do
  STARTSEC=$(date +%s)
  
  if [[ $ii -ge $LAST_REPORT_INTEGER ]]; then OFFSET=0; else OFFSET=1; fi

  SQL="insert into $TARGET_SCHEMA.$TARGET_TABLE select * from $SPIDER_SCHEMA.$SPIDER_TABLE where $INTEGER_COLUMN
       between
       $ii and (($ii + $PER_CHUNK) -$OFFSET); select format(ROW_COUNT(),0);"
